...
function TemporaryBonus:AddBonus(bonus,name,amount,time)
	...
	self.inst:DoTaskInTime(1+math.random()/2,function()
		if self.inst.userid then
			SendModRPCToClient(GetClientModRPC("Quest_System_RPC", "AddTempBoniToClient"),self.inst.userid,self.inst,num,bonus.."_"..amount)
		end
	end)
...
local function ChangeBoniClient(self,name,num)
	...
	if self.boni_num[num+1] ~= nil then
		...
		if tab and self.inst.userid then
			SendModRPCToClient(GetClientModRPC("Quest_System_RPC", "AddTempBoniToClient"),self.inst.userid,self.inst,num,tab.bonus.."_"..tab.amount)
